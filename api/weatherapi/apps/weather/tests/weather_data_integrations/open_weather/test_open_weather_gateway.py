...
from weatherapi.apps.weather.tests.weather_data_integrations.open_weather import OpenWeatherGatewayVCRMixin
from weatherapi.apps.weather.tests.weather_data_integrations.open_weather.fixtures import get_sample_one_call_response
...
class OpenWeatherGatewayTestCase(OpenWeatherGatewayVCRMixin, TestCase):
    ...
    def test_fetch_weather_data_returns_correct_data(self):
        weather_forecasts: OneCall = self.open_weather_gateway._fetch_weather_data(self.valid_long, self.valid_lat)
    ...
    def test_fetch_weather_data_handles_open_weather_exceptions(self):
        with patch.object(WeatherManager, 'one_call', side_effect=ConfigurationError()):
            with self.assertRaises(WeatherIntegrationGatewayException):
                self.open_weather_gateway._fetch_weather_data(self.valid_long, self.valid_lat)
    ...
    def test_extract_curated_weather_forecasts_curates_hourly_forecast_correctly(self):
        ...
        curated_forecasts = self.open_weather_gateway._extract_curated_weather_forecasts(OneCall.from_dict(one_call_json_response))
        # 1 current + 2 hourly
        ...
        self.assertEqual(datetime.fromisoformat('2021-01-24 22:00:00'), curated_forecasts[2].timestamp)

    def test_extract_curated_weather_forecasts_curates_current_forecast_correctly(self):
        one_call_json_response = get_sample_one_call_response(daily_forecasts=[], hourly_forecasts=[])
        curated_forecasts = self.open_weather_gateway._extract_curated_weather_forecasts(OneCall.from_dict(one_call_json_response))
        # 1 current
        ...
        self.assertEqual(datetime.fromisoformat('2021-01-24 21:38:30'), curated_forecasts[0].timestamp)
    ...
    def test_extract_curated_weather_forecasts_curates_daily_forecast_correctly(self):
        ...
        self.assertEqual(datetime.fromisoformat('2021-01-25 02:00:00'), curated_forecasts[1].timestamp)