...
class OpenWeatherGateway(WeatherDataIntegrationGateway):
    def __init__(self, open_weather_api_key):
        ...
        self.open_weather_client = OWM(
            self._API_KEY, config=self.config
        ).weather_manager()
        super().__init__()
    ...
    def __open_api_weather_to_weather_forecast(
        ...
        return WeatherForecast(
            ...
            visibility_distance=Decimal(owm_weather.visibility_distance)
            if owm_weather.visibility_distance
            else None,
            dew_point=Decimal(owm_weather.dewpoint),
            heat_index=Decimal(owm_weather.heat_index)
            if owm_weather.heat_index
            else None,
            utc_offset=owm_weather.utc_offset,
            uvi=Decimal(owm_weather.uvi) if owm_weather.uvi else None,
            sunrise=datetime.fromtimestamp(owm_weather.srise_time)
            if owm_weather.srise_time
            else None,
            sunset=datetime.fromtimestamp(owm_weather.sset_time)
            if owm_weather.sset_time
            else None,
            feels_like=feels_like,
            wind_gust=Decimal(owm_weather.wnd["gust"])
            if "gust" in owm_weather.wnd
            else None,
    ...
    def _fetch_weather_data(self, long, lat) -> dict:
        ...
        try:
            return self.open_weather_client.one_call(
                lat=lat, lon=long, exclude="minutely,alerts", units="metric"
            )
        except ConfigurationError as e:
            LOG.info(
                "ConfigurationError occurred while fetching weather data from Open Weather Map"
            )
            raise WeatherIntegrationGatewayException(
                f"{e}", e, WeatherIntegrationGatewayExceptionCode.CONFIGURATION.value
            )
        except APIRequestError as e:
            LOG.info(
                "APIRequestError occurred while fetching weather data from Open Weather Map"
            )
            raise WeatherIntegrationGatewayException(
                f"{e}", e, WeatherIntegrationGatewayExceptionCode.REQUEST.value
            )
        except APIResponseError as e:
            LOG.info(
                "APIResponseError occurred while fetching weather data from Open Weather Map"
            )
            raise WeatherIntegrationGatewayException(
                f"{e}", e, WeatherIntegrationGatewayExceptionCode.RESPONSE.value
            )
    ...
    def _extract_curated_weather_forecasts(self, weather_api_response: OneCall):
        ...
        weather_forecasts: List[WeatherForecast] = [
            self.__open_api_weather_to_weather_forecast(
                ...
                weather_api_response.lon,
                weather_api_response.lat,
            )
        ]
